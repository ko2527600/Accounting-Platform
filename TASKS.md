...
- [ ] DB tables with no corresponding API at all: `approval_workflows`, `approval_steps`, `report_definitions`, `attached_documents`. (`tax_rates`, `budgets`, `fiscal_periods`, and `recurring_transactions` are done as of 2026-07-25 - see above. `report_definitions`/`attached_documents` remain explicitly out of scope per the user's own scoping decision.)
