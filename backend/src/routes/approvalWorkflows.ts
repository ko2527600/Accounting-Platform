...
router.post('/', requireRole('Accountant'), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const approvalWorkflow = await approvalWorkflowService.createApprovalWorkflow(tenantId, requestedBy, req.body, actorFromRequest(req));
...
router.post('/:id/steps/:level/decide', requireRole('Accountant'), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const approvalWorkflow = await approvalWorkflowService.decideApprovalStep(tenantId, req.params.id, level, decision, comments, actorFromRequest(req));
