...
33. Invoice row actions-column overflow/crowding on `/invoices` - newly surfaced 2026-08-12 while live-verifying item 19's "Email Invoice" button. The actions cell (History/Record Payment/Collect via MoMo/Collect via Mobile Money/Credit Note, now +Email Invoice) already wraps and clips at normal viewport width with 5 buttons; a 6th makes it measurably worse - confirmed via Playwright screenshot, not just suspected. A "More actions" overflow menu (or icon-only buttons with tooltips, matching the existing History button's style) is the likely fix - small, self-contained UI task, no backend change needed.
