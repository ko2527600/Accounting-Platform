...
| **Multi-Factor Authentication (MFA/2FA)** | **GAP - real security exposure** | Confirmed via grep (2026-08-11) - zero matches for `mfa`/`2fa`/`totp`/`authenticator` anywhere in the backend or schema. No second factor on login at all, on a platform that moves real money (MoMo collections, bank feeds) - a documented industry-wide risk this app currently shares, not just a theoretical one. |
