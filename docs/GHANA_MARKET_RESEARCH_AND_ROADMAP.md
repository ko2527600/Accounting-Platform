...
1. **[DONE 2026-08-16]** ~~GRA E-VAT / SDC integration (invoice-time API call, security stamp, QR code on receipts)~~ - primary-source verification done 2026-08-01: model confirmed (real-time VSDC clearance, JSON/XML, GRA-issued digital signature/QR/SDC code embedded before the invoice can be shown to the customer, 24h offline grace period). A same-day 2026-08-16 scaffolding pass confirmed the real wire-level API contract wasn't publicly available (GRA hands it to a taxpayer only during their own onboarding, per guideline GRA/AG/2024/005 S10(e)) and built everything honestly buildable without it. Later the same day, the user supplied GRA's actual Postman API documentation ("GRA E-VAT API - VER 8.2") - the real spec that onboarding step would have produced - which resolved the blocker: `graEvatService.requestClearance()` now makes a real call to `POST https://vsdcstaging.vat-gh.com/vsdc/api/v1/taxpayer/{TIN}-{device}/invoice`, mapping this system's real Invoice/InvoiceItem/Customer/taxBreakdown data onto GRA's exact documented schema (not guessed - e.g. `calculationType` is always "EXCLUSIVE" because that's how this codebase actually computes tax). Since GRA assigns each taxpayer their own TIN/Device Number/security_key individually (unlike MoMo/Paystack's single platform-wide secret), this needed new per-tenant encrypted credential storage (AES-256-GCM), entered in Settings > GRA E-VAT. Scoped to new-sale clearance only - REFUND/credit-note-driven GRA reversal flows remain a real, separate, unbuilt follow-up (GRA's schema documents them; this codebase doesn't wire them up yet). See the 2026-08-16 STATUS.md entries (scaffolding, then real integration).
