...
- MoMo reconciliation / native checkout integration - `[ALREADY BUILT]` 2026-08-01 (MTN MoMo Collections, real "Request to Pay") and 2026-08-10 (TheTeller/PaySwitch - Telecel Cash, AirtelTigo Money, Zeepay, G-Money through one API, MTN kept on its own direct integration). True bank-style reconciliation isn't possible against either API (no bulk transaction-history endpoint) - each collection attempt is tracked individually and its result learned by polling. AirtelTigo Money's own standalone API remains independently unverified (only reachable via TheTeller today). See STATUS.md and Section 4 item 3.
...
- Fund accounting (restricted vs. unrestricted funds) for the NGO/institution segment - `[ALREADY BUILT]` 2026-08-10: new `Tenant.orgType` (BUSINESS/NONPROFIT) + `Fund` model + `fundId` threaded through Invoices/Vendor Bills/journal lines/ledgers, with a per-fund filter on P&L/Balance Sheet. Commercially unvalidated against a real prospective customer in this segment. See STATUS.md and Section 4 item 17.
- Hybrid-offline POS (local-first sale processing, async sync) - `[ALREADY BUILT]` 2026-08-08, scoped to sales-only per an explicit user decision (till open/close and voids still require connectivity): backend `clientTxnId` idempotency on `CashSale` (Phase 1) + a frontend `idb`-backed local queue with automatic/manual sync (Phase 2). A cashier can keep ringing up sales during an outage; a genuine sync conflict surfaces as "Needs Attention" rather than being silently dropped. Two related bugs found and fixed shortly after (2026-08-10): a reload during an outage stranding the cashier on the wrong screen, and the same item being oversold across two local carts within one outage. See STATUS.md and Section 4 item 16.
...
**1. Cost Creep and Pricing Structures** (subscription traps, feature gating, per-seat penalties) - **not currently applicable at the time of this entry, but worth designing correctly now.** No live billing/pricing model existed yet at this point (`Tenant.tier` was stored but only enforced one minor feature - custom fields, `tierEnforcementMiddleware.ts` - everything else, including multi-currency and batch invoicing, was available to every tenant regardless of tier; `dataExport.ts` explicitly documents a deliberate choice *not* to gate a core feature behind a tier). No per-seat pricing existed, so there was no current seat-penalty risk either. Flagged as a design constraint to hold when real billing eventually gets built (Section 4 item 7, then unscoped) - keep core capabilities (multi-currency, batch invoicing, inventory) *out* of any future paywall, matching the precedent `dataExport.ts` set. **Update 2026-08-15**: plan tiers shipped (Section 4 item 7, now done) - the constraint held: only Bank Reconciliation, Recurring Transactions, Budgets, Approval Workflows, and Custom Fields are Business-tier-gated; multi-currency, batch invoicing, inventory, and data export remain available on every tier. Seats are capped per tier, but no per-seat *pricing* exists yet (still no payment processor).
...
| Bank reconciliation matching logic | `[ALREADY BUILT 2026-08-12]` | New `GET /banking/transactions/:id/suggestions` finds real candidate Ledger entries (cash-equivalent accounts, exact amount match, ±10 day window). `POST /banking/reconcile` now requires and validates a real `ledgerId` match instead of blindly flipping status. See STATUS.md. |
...
| Late payment reminders (dunning emails) | `[ALREADY BUILT 2026-08-12]` | New `DunningReminderCronService` - daily sweep across every tenant's overdue, unpaid invoices, emailing a reminder and stamping a new `Invoice.lastReminderSentAt` with a 24-hour dedup guard. Blanket sweep, no per-tenant opt-in for v1. See STATUS.md. |
...
1. **[HONEST SCAFFOLDING BUILT 2026-08-16, real API contract still blocked]** GRA E-VAT / SDC integration (invoice-time API call, security stamp, QR code on receipts) - primary-source verification done 2026-08-01: model confirmed (real-time VSDC clearance, JSON/XML, GRA-issued digital signature/QR/SDC code embedded before the invoice can be shown to the customer, 24h offline grace period). The earlier "broken PDF tooling" blocker was a false lead - the 2026-08-16 pass successfully downloaded and fully read the official 16-page guidelines PDF (`gra.gov.gh/wp-content/uploads/2024/07/E-VAT-GUIDELINES_20240222.pdf`, GRA/AG/2024/005) once fetched with a browser User-Agent header; the real blocker is elsewhere and confirmed directly in the primary source: per Section 10(e), GRA hands its actual API documentation to a taxpayer only during their own bespoke onboarding (Relationship Manager, onboarding form, joint testing, sign-off, go-live) - there is no public self-serve spec to build the real wire-level call against, unlike MoMo/TheTeller/Paystack. Rather than invent field names (which this codebase's own "No Mock Data Ever"/"Spec-Driven Development" rules forbid, and which could mislead a real taxpayer), built everything that can be honestly built without guessing the contract: `Invoice` schema fields for the 5 legally-required certified-invoice elements (Section 7(3)), a `graEvatService.requestClearance()` that always throws with the real onboarding contacts rather than ever faking a success, a QR-code renderer, a real per-invoice "Request GRA Clearance" action that records the honest failure, and a Settings "GRA E-VAT" tab with the real onboarding process and GRA contact channels sourced directly from the guideline. **Still blocked** on GRA providing the actual API documentation directly to a taxpayer during their own onboarding - an external, non-technical blocker (contact GRA to begin: support@evatgra.zendesk.com or the GRA Contact Centre), not a gap in this codebase. See the 2026-08-16 STATUS.md entry.
...
4. **[DONE 2026-07-31, Investing section added 2026-08-15]** ~~Cash Flow Statement report (frontend + likely backend service function, mirroring how `getBalanceSheet`/`getProfitAndLoss` are already structured).~~ - built as a real indirect-method statement (Operating + Financing activities). Required a small but real schema addition first: `Account.isCashEquivalent` (auto-defaulted by name, overridable), since nothing previously distinguished cash/bank accounts from other assets. Originally shipped with **no Investing section** (documented simplification, since this schema had no fixed-asset/loan classification to separate capex from ordinary working capital) - closed 2026-08-15 alongside item 18 (Fixed Asset Management), which added `Account.isFixedAsset` to route capex changes into a real Investing Activities section instead of Operating. `netCashFromOperating + netCashFromInvesting + netCashFromFinancing` is guaranteed by double-entry bookkeeping to reconcile with the actual cash balance change (`cashTies`, mirroring the Balance Sheet's `isBalanced`). See STATUS.md.
...
7. **[DONE 2026-08-15]** ~~Real billing/plan enforcement tied to `tenant.tier` - separate track (see pricing-strategy discussion), not part of the Ghana-compliance research thread, but noted here since it came up in the same conversation.~~ - built as self-serve plan tiers: Shop (default) / Business (+ Bank Reconciliation, Recurring Transactions, Budgets, Approval Workflows, Custom Fields, 10 seats) / Enterprise (+ unlimited seats), gated via a new `requireTier()` middleware. Every tenant that existed before this shipped was grandfathered to Enterprise via a data migration, so introducing tiers took nothing away from anyone already using the platform. No self-serve billing/payment processor yet (explicit decision) - a platform admin sets a tenant's plan via a new Admin Core Engine control. See STATUS.md.
...
17. **[DONE 2026-08-10]** ~~Fund accounting (restricted vs. unrestricted fund tracking, e.g. for NGOs/schools/churches/cooperatives) - a new segment not previously covered in this doc; would need a `fund`/restriction dimension on transactions and independent per-fund balance sheets. Not yet validated against a real prospective customer in this segment - the 2026-07-25 target-market discussion this session focused on retail/SME, not non-profits, so worth confirming this is actually a market we want before scoping.~~ - built per explicit user decision: new `Tenant.orgType` (`BUSINESS`/`NONPROFIT`, chosen once at onboarding) drives nav (hides POS/Inventory, adds a "Funds" section for nonprofits); a new `Fund` model plus `fundId` threaded through Invoices/Vendor Bills/journal lines/ledgers, with `getProfitAndLoss`/`getBalanceSheet` gaining an optional per-fund filter. Scope deliberately limited to core fund tracking + per-fund reports - no donor CRM, receipting, or grant-report templates. Found and fixed a real bug along the way: voiding a fund-tagged posted entry (and the equivalent credit/debit-note reversal paths) was dropping the `fundId` when building the reversal, which would have permanently overstated that fund's balance in reports - fixed and covered by a regression test. This nonprofit segment remains commercially unvalidated against a real prospective customer. See STATUS.md.
18. **[DONE 2026-08-15]** ~~Fixed Asset Management (register with purchase date/initial valuation/serial ID per item, plus Straight-Line/Reducing-Balance depreciation calculation) - newly surfaced 2026-08-11 from a user-supplied competitor/ERP feature checklist. Confirmed a genuine gap via grep, not previously tracked in this doc. Would also unblock the Cash Flow Statement's currently-missing Investing section (item 4 above), since there's no capex/asset classification today to separate from ordinary working capital.~~ - built exactly as scoped, closing item 4's Investing-section gap too: a new `FixedAsset` register (purchase date, cost, residual value, serial number, method) posts a real acquisition journal entry on creation and a real monthly depreciation entry via a daily cron (Straight-Line or Reducing-Balance, capped at `cost - residualValue`). New `Account.isFixedAsset` flag routes those accounts' balance changes into a real Investing Activities section on the Cash Flow Statement instead of Operating - the Balance Sheet needed no code changes at all, since Fixed Asset/Accumulated Depreciation are just ordinary ASSET accounts. See STATUS.md.
...
20. **[DONE 2026-08-15]** ~~Customer credit limits (block or warn when a new invoice would push a customer's outstanding balance past a set ceiling) - newly surfaced 2026-08-11. Needs a schema addition (`Customer.creditLimit` or similar) plus an enforcement check at invoice creation - small-to-medium lift.~~ - built exactly as scoped: new optional `Customer.creditLimit`, enforced as a hard block (not just a warning) at `POST /invoices` - current outstanding balance (SENT/PARTIALLY_PAID invoices) plus the new invoice's total must not exceed the limit. Skipped entirely for an idempotent `clientTxnId` replay so a retried request never double-counts against its own limit. See STATUS.md.
21. **[DONE 2026-08-12]** ~~Late payment reminders / dunning emails (scheduled nudge to customers with overdue invoices) - newly surfaced 2026-08-11. Architecturally close to the existing scheduled-report cron (`ScheduledEmailCronService`) - likely a new job in the same pattern, checking `Invoice.dueDate` against today rather than a report schedule.~~ - built as `DunningReminderCronService`, a daily 9am UTC sweep (blanket, no per-tenant opt-in) rather than a `ScheduledEmailCronService`-style per-tenant `ReportSchedule`, matching `RecurringTransactionCronService`/`FxRateCronService`'s "always run for what's due" shape instead. New `Invoice.lastReminderSentAt` field with a 24-hour dedup guard. See STATUS.md.
22. **[DONE 2026-08-15]** ~~Embedded payment link on invoices (Stripe-style pay-now link/button on the invoice document) - newly surfaced 2026-08-11. Needs a payment-gateway decision first (Stripe itself, or a Ghana-relevant equivalent - Paystack/Flutterwave support Ghana and are more locally standard than Stripe; worth a research pass before picking, consistent with this doc's Ghana-first bias elsewhere e.g. MTN MoMo over a generic global processor).~~ - Paystack chosen (explicit user decision when this was scoped for building). Built following this codebase's established env-gated integration pattern exactly (`PAYSTACK_SECRET_KEY` unset → 503, never a faked link): an accountant generates a hosted-checkout link for an invoice's outstanding balance, shares it with the customer, then verifies payment directly against Paystack's real verify API (never trusts a client-side "paid" claim) before the invoice is marked paid - reuses the same shared `invoicePaymentService` every other payment method already posts through. Scoped to this generate-then-verify flow (matching TheTeller's existing "Check Status" pattern), not a public unauthenticated webhook/customer-return page. See STATUS.md.
23. **[DONE 2026-08-15]** ~~Recurring customer invoices / subscription billing - newly surfaced 2026-08-11, and a genuine architectural mismatch rather than a clean gap: the existing `RecurringTransaction` engine already has the scheduling/cron machinery, but only ever produces a `JournalEntry`, never an `Invoice`, and has no `customerId` concept. Extending it to optionally spawn an `Invoice` (or a parallel `RecurringInvoice` entity reusing the same cron) is likely a smaller lift than building AR subscription billing from scratch - worth scoping as "extend" rather than "build new."~~ - built exactly as scoped: a new parallel `RecurringInvoice` entity (not an extension of `RecurringTransaction`), with its own daily cron generating a real Simple Invoice (no itemized/stock-deducting lines) from a JSON line-item template, correctly handling optional tax, cross-currency conversion (honest fallback if FX isn't configured), and an optional end date. See STATUS.md.
...
25. **[DONE 2026-08-15]** ~~Purchase Order (PO) entity + PO-vs-bill matching, and auto-generating a PO when an item crosses its `reorderLevel` (closing the "alert-only" half of item 26 below) - newly surfaced 2026-08-11. A new core entity, likely a prerequisite for both AP matching and the reorder-threshold gap - worth scoping as its own foundational piece rather than bundling into either dependent feature.~~ - built as a new `PurchaseOrder`/`PurchaseOrderLine` entity (DRAFT→SENT→BILLED/CANCELLED) with lightweight rule-based quantity/cost variance flagging against the bill eventually raised against it (not a strict blocking 3-way match). The reorder half is a "Generate Reorder POs" action an accountant triggers on demand (one PO per preferred vendor, for every item at/below its reorder level) - honestly scoped as on-demand generation, not a fully automatic background job that fires the instant a threshold is crossed. See STATUS.md.
26. **[DONE 2026-08-15]** ~~Automated reorder alerts + auto-PO-on-threshold - newly surfaced 2026-08-11. `InventoryItem.reorderLevel` already exists and drives a frontend low-stock badge, so the "alert" half is a smaller lift (a backend notification job checking the existing field); the "auto-generate a PO" half is blocked on item 25.~~ - the "alert" half already existed (frontend low-stock badge); the "auto-generate a PO" half is now built via item 25's on-demand "Generate Reorder POs" action, not a background cron that fires automatically on threshold crossing - see item 25's note on that honest scoping. See STATUS.md.
27. **[DONE 2026-08-15]** ~~Automated vendor payment scheduling (batch bill payouts on their due dates) - newly surfaced 2026-08-11. Same architectural pattern/mismatch as recurring customer invoices (item 23) - the existing cron only produces journal entries, not bill payments.~~ - built as a real daily cron (`VendorPaymentSchedulingCronService`, 8am UTC) that auto-pays any bill whose `scheduledPaymentDate` has arrived, posting through the exact same `payVendorBill()` logic the manual "Pay Bill" button uses (extracted into a shared service so the two paths can never drift). See STATUS.md.
28. **[DONE 2026-08-15]** ~~AP/AR aging analysis (30/60/90-day overdue buckets for vendor bills and customer invoices) - newly surfaced 2026-08-11, broader than the checklist's AP-only framing since neither side has it. Likely a lighter lift than it sounds - `dueDate` already exists on `Invoice`/`VendorBill`, this is mostly a new report bucketing existing data, similar in shape to the KPI dashboard.~~ - built as `/reports/aging`: current/1-30/31-60/61-90/90+ day buckets for both AR (respects partial payments via `total - amountPaid`) and AP (full amount, since vendor bills in this schema are all-or-nothing). See STATUS.md.
29. **[DONE 2026-08-12]** ~~Real bank-transaction matching logic for `POST /banking/reconcile` - newly surfaced 2026-08-11. Today's endpoint just flips status on a client-chosen pair with zero matching algorithm; even simple rule-based matching (amount + date window) would be a real improvement before any "AI" framing is warranted. Worth fixing regardless of this checklist, since the current behavior doesn't match its own "reconcile" naming.~~ - built as rule-based amount+date matching: new `GET /banking/transactions/:id/suggestions` finds candidate Ledger entries on cash-equivalent accounts within a ±10-day window whose exact amount matches, and `POST /banking/reconcile` now requires and validates a real `ledgerId` match (400 on amount mismatch, 409 if the ledger row is already claimed) instead of blindly flipping status. See STATUS.md.
30. **[DONE 2026-08-15]** ~~Petty cash logs (ad-hoc minor office cash disbursements, distinct from `CashTill`'s POS sales and `ExpenseClaim`'s employee reimbursement workflow) - newly surfaced 2026-08-11.~~ - built as a new `PettyCashEntry` model; each disbursement/replenishment posts a real 2-line journal entry (mirrors the existing Contra Voucher pattern) and the entry history shows a running balance, oldest-first. See STATUS.md.
31. **[EXPLICITLY SKIPPED 2026-08-15]** ~~Check printing & generation - newly surfaced 2026-08-11. Flagged low-priority: paper cheques are uncommon in Ghana SME payments versus MoMo/bank transfer/cash - shouldn't be scoped ahead of the other items above without real customer demand evidence.~~ - when this list was scoped for building, the user was asked whether to include it and explicitly chose to skip it, on the same low-priority/no-demand-evidence reasoning already flagged here. Left unbuilt on purpose, not forgotten - revisit only if real customer demand for paper cheques surfaces.
