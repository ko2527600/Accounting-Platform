...
2. Layered Ghana tax levy breakdown (NHIL/GETFund/COVID or WHT/VAT) on invoices - **RESOLVED 2026-08-01** (confirmed directly on `gra.gov.gh`): flat **20%** total = 15% VAT + 2.5% NHIL + 2.5% GETFund Levy, both re-coupled into the VAT base (input tax credit restored) under VAT Act 2025 (Act 1151), effective January 1, 2026; the 1% COVID-19 Levy is abolished. The old cascading NHIL/GETFund/COVID model (Finza's/Webhuk's structure) is superseded - ready to scope for real now, no further tax-rate research needed.
3. Mobile Money account type + reconciliation (MTN MoMo, Vodafone Cash, AT Money) - primary-source verification done 2026-08-01: MTN MoMo and Telecel Cash (rebranded Vodafone Cash) both have real developer APIs with self-serve sandboxes (production needs KYC approval). Also found a real single-integration alternative, TheTeller (PaySwitch Ltd, est. 2015), covering MTN+Vodafone/Telecel+AirtelTigo through one API - closer to Mono's role for bank feeds than integrating 3 telcos separately. AirtelTigo Money not independently verified yet. See the 2026-08-01 research log entry.
